...
const { Identity: ActualIdentity } = jest.requireActual('@src/core/generics/Identity');

jest.mock('@src/core/generics/Identity', () => ({
  Identity: jest.fn().mockImplementation((id?: string) => new ActualIdentity(id)),
}));
...
describe('Entity', () => {
  ...
  it('Deve criar uma entidade com um novo ID se nenhum for fornecido', () => {
    ...
    expect(entity.identityId.id).toEqual(expect.stringMatching(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/));
  });
